...
import { handleXNhanSearch } from "./xnhan/search.js";
import { handleSecuritiesRequest } from "./securities/api.js";
import { resolveSecuritiesPage } from "../shared/securities/routes.js";
...
async function serveProductShell(request, env, shellPath = XNHAN_ASSET_PATH) {
  ...
  return withHtmlCspNonce(localAssetResponse(assetResponse, env));
}

function localAssetResponse(response, env) {
  if (env.SECURITIES_LOCAL_MODE !== "true") return response;
  const headers = new Headers(response.headers);
  // Local builds replace assets in place. Never cache a transient missing file.
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith("/api/securities/") || url.pathname === "/api/securities") {
      return handleSecuritiesRequest(request, env, ctx);
    }
    ...
    const securitiesPage = resolveSecuritiesPage(url);
    if (securitiesPage) {
      if (request.method !== "GET" && request.method !== "HEAD") {
        return errorResponse("method_not_allowed", 405, crypto.randomUUID(), { Allow: "GET, HEAD" });
      }
      if (!securitiesPage.redirect) return serveProductShell(request, env, securitiesPage.asset);
      return new Response(null, { status: 308, headers: {
        Location: securitiesPage.redirect, "Cache-Control": "no-store",
    ...
    if (url.pathname === XNHAN_CANONICAL_PATH) {
      return serveProductShell(request, env);
    }
    if (
    ...
    if (url.pathname === XNHAN_ABOUT_CANONICAL_PATH) {
      return serveProductShell(request, env, XNHAN_ABOUT_ASSET_PATH);
    }
