...
import { xnhanAboutContent } from "../src/xnhan/xnhan-about-content.js";
import { xnhanContent } from "../src/xnhan/xnhan-content.js";

const root = new URL("../", import.meta.url);
const wranglerSource = await readFile(new URL("wrangler.jsonc", root), "utf8");
const wrangler = JSON.parse(wranglerSource);
const packageJson = JSON.parse(await readFile(new URL("package.json", root), "utf8"));
const securityTxtSource = await readFile(new URL("public/.well-known/security.txt", root), "utf8");
const llmsTxtSource = await readFile(new URL("public/llms.txt", root), "utf8");
const generatedBindingsSource = await readFile(new URL("worker-configuration.d.ts", root), "utf8");
...
const securitiesMigrationSource = await readFile(
  new URL("d1-migrations/0002_securities_shared_history.sql", root),
  "utf8",
);
...
const publicProductVersionPattern = /\b(?:phiên\s+bản|v|version)\s*#?\s*\d+(?:\.\d+)*\b/giu;
...
const nonRenderedTechnicalReferenceRules = [
  ...
  {
    locationPattern: /^src\/person-schema\.js#(?:en|vi):(?:@context|@id|sameAs\.\d+|url)$/u,
  ...
  {
    locationPattern: /^(?:index|xnhan|xnhan-about)\.html:jsonLd\.\d+(?:\.[^.]+)*\.doi$/u,
    valuePattern: doiPattern,
  },
  {
    locationPattern: /^(?:index|xnhan|xnhan-about)\.html:jsonLd\.\d+(?:\.[^.]+)*\.identifier$/u,
    valuePattern: technicalIdentifierPattern,
  },
  {
...
const publicHtmlAttributeNames = ["alt", "aria-description", "aria-label", "placeholder", "title"];
...
function collectHtmlPublicCopy({ fileName, source }) {
  ...
  for (const match of source.matchAll(/<script\b[^>]*>[\s\S]*?<\/script>/giu)) {
    ...
    const mediaType = readHtmlAttribute(openingTag, "type")?.split(";", 1)[0].trim().toLowerCase();
    if (mediaType !== "application/ld+json") continue;

    const serialized = block.replace(/^<script\b[^>]*>/iu, "").replace(/<\/script>$/iu, "");
    let structuredData;
    try {
    ...
    collectStringEntries(structuredData, fileName, ["jsonLd", String(jsonLdIndex)], entries);
    jsonLdIndex += 1;
  }
...
function isAllowlistedNonRenderedTechnicalReference({ source, path, value }) {
  ...
  return nonRenderedTechnicalReferenceRules.some(
    (rule) => rule.locationPattern.test(location) && rule.valuePattern.test(value.trim()),
  );
}
...
test("keeps RFC 9116 discovery metadata actionable and short lived", () => {
  assert.match(securityTxtSource, /^Contact: mailto:tranthiennhan\.work@gmail\.com$/mu);
  assert.match(
    securityTxtSource,
...
test("keeps the bounded production assets, AI, metrics, visitor D1, daily cron, and rate-limit bindings", () => {
  ...
  assert.match(rateLimits.SECURITIES_RATE_LIMIT.namespace_id, /^[1-9]\d*$/u);
  assert.notEqual(
  ...
  assert.deepEqual(rateLimits.SECURITIES_RATE_LIMIT.simple, {
    limit: 20,
    period: 60,
  });
...
test("enables shared Securities storage while preserving dashboard-owned model vars", () => {
  assert.equal(wrangler.keep_vars, true);
  assert.deepEqual(wrangler.vars, { SECURITIES_SHARED_MODE: "true" });
  assert.match(generatedBindingsSource, /SECURITIES_RATE_LIMIT: RateLimit/u);
  assert.match(generatedBindingsSource, /SECURITIES_SHARED_MODE: "true"/u);
  assert.doesNotMatch(
    wranglerSource,
    /XNHAN_OPENROUTER_SEARCH_TRANSPORT|XNHAN_OPENROUTER_REASONING_EFFORT/u,
  );
  ...
  assert.equal(Object.hasOwn(wrangler.vars ?? {}, "XNHAN_OPENAI_MODEL_DISPLAY_NAME"), false);
  assert.equal(Object.hasOwn(wrangler.vars ?? {}, "XNHAN_OPENROUTER_MODEL_DISPLAY_NAME"), false);
  assert.doesNotMatch(
    wranglerSource,
    /XNHAN_OPENROUTER_MAX_(?:PROMPT|COMPLETION)_PRICE_PER_M|max_price/iu,
  );
  assert.doesNotMatch(generatedBindingsSource, /XNHAN_(?:OPENAI|OPENROUTER)_MODEL/u);
  ...
  assert.match(runtimeModelBindingsSource, /XNHAN_OPENAI_MODEL_DISPLAY_NAME\?: string/u);
  assert.match(runtimeModelBindingsSource, /XNHAN_OPENROUTER_MODEL_DISPLAY_NAME\?: string/u);
  assert.doesNotMatch(
    wranglerSource,
...
test("the production D1 migration adds the complete shared Securities schema", () => {
  const database = new DatabaseSync(":memory:");
  try {
    database.exec(visitorMigrationSource);
    database.exec(securitiesMigrationSource);
    ...
    for (const table of [
      ...
      "securities_requests",
      "securities_jobs",
      "securities_source_imports",
      "securities_source_datasets",
      "securities_source_checks",
    ])
...
test("keeps owner-controlled public copy free of sequential product version labels", () => {
  const publicCopyEntries = [
    ...
    ...collectStringEntries(xnhanContent, "src/xnhan/xnhan-content.js"),
    ...collectStringEntries(xnhanAboutContent, "src/xnhan/xnhan-about-content.js"),
  ...
  assert.deepEqual([...new Set(publicCopyEntries.map(({ source }) => source))].sort(), [
    "index.html",
    "src/content.js",
...
test("gates a production deployment behind tests and a Wrangler dry run", () => {
  ...
  assert.equal(packageJson.scripts.build, "vite build && node scripts/build-localized-shells.mjs");
  assert.equal(
  ...
  assert.equal(
    packageJson.scripts["test:xnhan"],
    'pnpm build && node --test "tests/xnhan/*.test.mjs"',
  );
  assert.equal(packageJson.scripts["check:cloudflare-types"], "wrangler types --check");
  assert.equal(
...
test("treats Wrangler dry-run warnings as a failed production gate", () => {
  ...
  assert.deepEqual(
    assessWranglerDryRun({ status: 0, stdout: "WARNING: ignored field", stderr: "" }),
    {
      ok: false,
  ...
  assert.deepEqual(
    assessWranglerDryRun({ status: 0, stdout: "", stderr: "Unexpected fields found" }),
    {
      ok: false,
      reason: "configuration_warning",
    },
  ...
  assert.deepEqual(
    assessWranglerDryRun({ status: null, stdout: "", stderr: "", error: new Error("spawn") }),
    {
      ok: false,
      reason: "spawn_error",
    },
