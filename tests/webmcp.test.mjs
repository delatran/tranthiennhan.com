...
function createOverviewDocumentFixture(overview) {
  ...
  if (overview.securitiesProduct) elements.set("securities-product-title", element(overview.securitiesProduct.name));
  const anchors = [
    overview.product.route,
    overview.product.aboutRoute,
    ...(overview.securitiesProduct ? [overview.securitiesProduct.route] : []),
...
test("reads one exact, trusted, locale-synchronized overview of all public portfolio sections", async () => {
  for (const locale of PORTFOLIO_WEBMCP_LOCALES) {
    ...
    assert.deepEqual(output.securitiesProduct, { name: "Nhân for Securities", route: "/securities" });
    assert.deepEqual(
...
test("overview accepts the application's localized product hrefs without accepting changed targets", () => {
  for (const locale of PORTFOLIO_WEBMCP_LOCALES) {
    const overview = portfolioOverviewForLocale(locale);
    const fixture = createOverviewDocumentFixture(overview);
    const options = { documentObject: fixture.documentObject, locationObject: fixture.locationObject };
    const routes = [overview.product.route, overview.product.aboutRoute, overview.securitiesProduct.route];
    routes.forEach((route, index) => fixture.anchors[index].setAttribute("href", `${route}?lang=${locale}`));
    assert.equal(portfolioOverviewMatchesDocument(overview, options), true);

    for (const invalid of [
      `/xnhan?lang=${locale === "vi" ? "en" : "vi"}`,
      `/xnhan?lang=${locale}&lang=${locale}`,
      `/xnhan?lang=${locale}&redirect=https://example.com`,
      `https://example.com/xnhan?lang=${locale}`,
      `//example.com/xnhan?lang=${locale}`,
      `/xnhan?lang=${locale}#unrequested-state`,
      `/xnhan/about?lang=${locale}`,
    ]) {
      fixture.anchors[0].setAttribute("href", invalid);
      assert.equal(portfolioOverviewMatchesDocument(overview, options), false, invalid);
    }
    fixture.anchors[0].setAttribute("href", `/xnhan?lang=${locale}`);
    assert.equal(portfolioOverviewMatchesDocument(overview, options), true);
    fixture.anchors[2].setAttribute("href", "/nhan-for-securities");
    assert.equal(portfolioOverviewMatchesDocument(overview, options), false);
    fixture.anchors[2].setAttribute("href", `/securities?lang=${locale}`);
    fixture.elements.get("securities-product-title").textContent = "Outdated product name";
    assert.equal(portfolioOverviewMatchesDocument(overview, options), false);
  }
});

test("optional Securities overview keeps the existing X Nhân product shape compatible", async () => {
  const { securitiesProduct, ...legacy } = portfolioOverviewForLocale("en");
  assert.equal(securitiesProduct.name, "Nhân for Securities");
  const legacyTools = createPortfolioWebMcpTools(createActions({ readPortfolioOverview: async () => legacy }));
  assert.deepEqual(await legacyTools[4].execute({}), legacy);
  const fixture = createOverviewDocumentFixture(legacy);
  assert.equal(portfolioOverviewMatchesDocument(legacy, { documentObject: fixture.documentObject, locationObject: fixture.locationObject }), true);

  let getterRead = false;
  for (const invalid of [
    null,
    { name: "Nhân Analyst", route: "/securities" },
    { name: "Nhân for Securities", route: "/securities?lang=en" },
    { name: "Nhân for Securities", route: "/securities", status: "deployed" },
    Object.defineProperty({ route: "/securities" }, "name", { enumerable: true, get() { getterRead = true; return "Nhân for Securities"; } }),
  ]) {
    const tools = createPortfolioWebMcpTools(createActions({ readPortfolioOverview: async () => ({ ...legacy, securitiesProduct: invalid }) }));
    await assert.rejects(tools[4].execute({}), TypeError);
  }
