...
} from "../../src/xnhan/use-xnhan-about-webmcp.js";
...
function createOverviewDocument(overview) {
  ...
  const single = new Map([
    ...
    ["[data-about-thesis]", textElement(overview.hero.thesis)],
...
test("keeps the X Nhân product story bilingual and structurally aligned", () => {
  ...
  assert.match(xnhanAboutContent.vi.how.intro, /câu hỏi cụ thể.+bài đăng được liên kết.+kiểm tra ngữ cảnh/iu);
  assert.match(xnhanAboutContent.en.how.intro, /specific question.+linked posts.+check their context/iu);
  assert.doesNotMatch(xnhanAboutContent.vi.how.intro, /backend|OpenAI|OpenRouter/iu);
  assert.doesNotMatch(xnhanAboutContent.en.how.intro, /backend|OpenAI|OpenRouter/iu);
...
test("renders one editorial About page without mounting either assistant", async () => {
  const [app, styles, webMcpAdapter] = await Promise.all([
    ...
    source("src/xnhan/use-xnhan-about-webmcp.js"),
  ]);
  ...
  assert.match(app, /<p className="xnhan-about-thesis" data-about-thesis>\{copy\.hero\.thesis\}<\/p>/u);
  assert.match(app, /<ol className="xnhan-about-reading-path">/u);
  assert.doesNotMatch(app, /xnhan-about-hero-aside|xnhan-about-signal|<blockquote>/u);
  assert.match(webMcpAdapter, /"\[data-about-thesis\]",\s*overview\.hero\.thesis/u);
  assert.doesNotMatch(webMcpAdapter, /hero-aside|blockquote/u);
  const heroRule = styles.match(/\.xnhan-about-hero\s*\{[^}]*\}/su)?.[0];
  assert.ok(heroRule);
  assert.doesNotMatch(heroRule, /min-height:|\bdvh\b/u);
  assert.match(heroRule, /align-items:\s*end/u);
  const heroContextRule = styles.match(/\.xnhan-about-hero-context\s*\{[^}]*\}/su)?.[0];
  assert.ok(heroContextRule);
  assert.doesNotMatch(heroContextRule, /background:|border-radius:|box-shadow:/u);
  assert.match(styles, /\.xnhan-about-hero h1\s*\{[^}]*font-size:\s*clamp\(3rem, 5\.8vw, 5\.4rem\)/su);
  ...
  assert.doesNotMatch(styles, /html\[lang="en"\] \.xnhan-about-(?:hero h1|title-line)/u);
  assert.match(styles, /@media \(max-width:\s*52\.5rem\)/u);
  assert.doesNotMatch(styles, /xnhan-about-signal-pulse/u);
  assert.match(
    styles,
...
test("builds the read-only About snapshot from public copy and validates its DOM commit", () => {
  ...
  documentObject.single.delete("[data-about-thesis]");
  assert.equal(
    xNhanAboutOverviewMatchesDocument(overview, {
      documentObject,
      locationObject,
    ...
    false,
    "the overview must wait until its thesis is present in the current DOM",
  );
  documentObject.single.set("[data-about-thesis]", textElement("stale thesis"));
  assert.equal(
    xNhanAboutOverviewMatchesDocument(overview, {
      documentObject,
      locationObject,
    ...
    false,
    "a semantic selector must still match the current thesis value",
  );
  documentObject.single.set("[data-about-thesis]", textElement(overview.hero.thesis));
  assert.equal(
    xNhanAboutOverviewMatchesDocument(overview, {
      documentObject,
      locationObject,
...
test("builds a product-specific canonical shell for /xnhan/about", async () => {
  ...
  assert.match(sourceShell, /src\/xnhan\/xnhan-about-main\.jsx/u);
