...
} from "../../shared/xnhan/contracts.js";
import {
  formatMetric,
  ...
  XNHAN_PROVIDERS,
  xnhanContent,
} from "../../src/xnhan/xnhan-content.js";
import {
...
} from "../../src/xnhan/xnhan-stream.js";
import { isXNhanModelId } from "../../src/xnhan/xnhan-model-id.js";
import {
  formatXNhanAnswerForClipboard,
  supportingAnswerSources,
} from "../../src/xnhan/xnhan-copy.js";
import {
  readExplicitXNhanLocale,
  readInitialXNhanLocale,
  replaceXNhanLocaleInUrl,
  writeStoredXNhanLocale,
} from "../../src/xnhan/xnhan-locale.js";
import {
  xNhanHref,
...
import {
  createXNhanChatTurn,
  reduceXNhanTurns,
} from "../../src/xnhan/xnhan-session-state.js";
import { executeXNhanSearchRequest } from "../../src/xnhan/xnhan-search-request.js";
import {
  classifyXNhanSearchFailure,
  isCurrentXNhanSearchRequest,
  scheduleXNhanSearchTimeout,
  XNHAN_REQUEST_TIMEOUT_MS,
} from "../../src/xnhan/xnhan-search-lifecycle.js";
import { createXNhanSearchStatus } from "../../src/xnhan/xnhan-search-status.js";
import { createXNhanWebMcpSnapshot } from "../../src/xnhan/xnhan-webmcp-snapshot.js";
...
test("keeps path matching exact and gives every product a route-specific entry", async () => {
  ...
  const [main, xnhanMain, aboutMain, xnhan, about, portfolio, portfolioRoute] = await Promise.all([
    source("src/main.jsx"),
    source("src/xnhan/xnhan-main.jsx"),
    source("src/xnhan/xnhan-about-main.jsx"),
  ...
  assert.match(main, /import "\.\/fonts\.css"/u);
  assert.match(main, /import "\.\/base\.css"/u);
  for (const entry of [xnhanMain, aboutMain]) {
...
test("normalizes every client and Worker query through one bounded contract", async () => {
  ...
  const [app, session, worker] = await Promise.all([
    source("src/xnhan/XNhanApp.jsx"),
    source("src/xnhan/use-xnhan-search-session.js"),
    source("worker/xnhan/search.js"),
  ]);
  ...
  assert.match(app, /className="xnhan-submit"[\s\S]*?disabled=\{!queryIsValid\}/u);
...
test("keeps the X Nhân HTML shell product-specific and free of portfolio Person data", async () => {
  ...
  assert.match(sourceShell, /src\/xnhan\/xnhan-main\.jsx/u);
...
test("keeps literal retrieval text identical across response, UI copy, and WebMCP source state", async () => {
  ...
  const [answerSource, turnSource, snapshotSource] = await Promise.all([
    source("src/xnhan/XNhanAnswer.jsx"),
    source("src/xnhan/XNhanTurn.jsx"),
    source("src/xnhan/xnhan-webmcp-snapshot.js"),
  ]);
...
test("keeps completed activity collapsible and removes presentation-only summary markup", async () => {
  const [component, turn] = await Promise.all([
    source("src/xnhan/XNhanActivity.jsx"),
    source("src/xnhan/XNhanTurn.jsx"),
  ]);
...
test("keeps exactly three fixed bilingual starter prompts with no trend runtime", async () => {
  const [app, contentSource, styles] = await Promise.all([
    source("src/xnhan/XNhanApp.jsx"),
    source("src/xnhan/xnhan-content.js"),
...
test("makes long research interruptible without inventing persistence or provider cancellation", async () => {
  const [app, session, sessionState, autosize, turn, activity, styles, turnStyles] = await Promise.all([
    source("src/xnhan/XNhanApp.jsx"),
    source("src/xnhan/use-xnhan-search-session.js"),
    source("src/xnhan/xnhan-session-state.js"),
  ...
  const composer = app.slice(app.indexOf('<form className="xnhan-composer"'));
  assert.match(
    composer,
    /\{busy \? \(\s*<button\s+className="xnhan-stop"\s+type="button"[\s\S]*?onClick=\{handleStopSearch\}[\s\S]*?\) : \(\s*<button\s+className="xnhan-submit"/u,
    "an active search must expose Stop in the persistent composer instead of a disabled submit",
  );
  const stopButton = composer.match(/<button\s+className="xnhan-stop"[^>]*>/u)?.[0];
  assert.ok(stopButton);
  assert.doesNotMatch(stopButton, /\bdisabled\b/u);
  assert.match(stopButton, /aria-label=\{copy\.progress\.stop\}/u);
...
test("shares the owner portrait across X Nhân brand and assistant marks", async () => {
  const [app, about, turn, logo, baseStyles, styles, aboutStyles, turnStyles] = await Promise.all([
    ...
    source("src/xnhan/XNhanAboutApp.jsx"),
    source("src/xnhan/XNhanTurn.jsx"),
    source("src/xnhan/XNhanLogo.jsx"),
    source("src/base.css"),
    source("src/xnhan/xnhan.css"),
...
test("wires OpenRouter-default search, a bounded chat transcript, collapsible sources, and route-scoped WebMCP", async () => {
  const [app, session, request, lifecycle, searchStatus, sessionState, snapshot, turn, answer, hook, styles, turnStyles, globalStyles] = await Promise.all([
    source("src/xnhan/XNhanApp.jsx"),
    source("src/xnhan/use-xnhan-search-session.js"),
    source("src/xnhan/xnhan-search-request.js"),
    source("src/xnhan/xnhan-search-lifecycle.js"),
    source("src/xnhan/xnhan-search-status.js"),
    source("src/xnhan/xnhan-session-state.js"),
    source("src/xnhan/xnhan-webmcp-snapshot.js"),
    source("src/xnhan/XNhanTurn.jsx"),
    source("src/xnhan/XNhanAnswer.jsx"),
    source("src/xnhan/use-xnhan-webmcp.js"),
    source("src/xnhan/xnhan.css"),
    source("src/xnhan/xnhan-turn.css"),
  ...
  await assert.rejects(source("src/xnhan/XNhanProviderPicker.jsx"), { code: "ENOENT" });
