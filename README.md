...
![The English portfolio home page, with a typographic introduction, project highlights, and the Ask Nhân launcher.](docs/assets/portfolio-preview.png)

The Kinetic Atelier interface uses warm forest typography, open grids, thin rules, and restrained moss accents. Identity, project evidence, and a clear reading flow carry the visual hierarchy. Project summaries keep evidence and limitations visible, while native disclosures open the deeper case studies. Responsive layouts, keyboard focus, reduced motion, and readable chat transcripts carry through the portfolio and both X Nhân pages.
...
| Nhân for Securities  | `/securities`   | Shared company research, detailed evidence and exact-revision exports |

The [Nhân for Securities guide](docs/securities.md) covers its local workspace,
shared-production D1 contract, public-report collector, exact OpenRouter model
and WebMCP workflow. Start the dedicated local mode with `pnpm dev:securities`.
The checked production configuration does not claim a live deployment.
...
Cloudflare bindings for static assets, Workers AI, D1, Analytics Engine, and five rate limiters are declared in [`wrangler.jsonc`](wrangler.jsonc). `SECURITIES_SHARED_MODE` maps the Securities store to separate tables in the production D1 binding; it does not create accounts, private libraries, or enable live model spend. The checked generated contract in [`worker-configuration.d.ts`](worker-configuration.d.ts) is intentional source, not disposable build output.
...
| `pnpm test:xnhan`             | Client build and focused X Nhân UI, WebMCP, provider, ranking, and streaming contracts pass |
...
├── shared/              # Runtime independent contracts and answer language helpers
│   └── xnhan/           # X Nhân contracts, locales, labels, and public route helpers
├── src/                 # Portfolio, Ask Nhân, and common browser components
│   └── xnhan/           # X Nhân and About UI, hooks, CSS, sessions, and WebMCP
├── tests/               # Node regression and repository contract tests
│   └── xnhan/           # X Nhân regression suite
├── worker/              # Common router, Ask Nhân, HTTP, rate limits, and analytics
│   └── xnhan/           # Search handler, provider adapters, prompts, and ranking
├── index.html
├── xnhan.html
├── xnhan-about.html
├── vite.config.mjs
...
X Nhân is a module within this repository, with one package, build, and deployment. Its frontend, backend, shared contracts, and tests have dedicated directories. Portfolio navigation uses the public route helper in `shared/xnhan/routes.js`; it does not import X Nhân UI internals. See [module ownership and dependencies](docs/architecture.md#source-ownership-and-dependencies) before moving code across these boundaries.
...
- Nhân for Securities production dossiers and completed follow-up conversations use one shared D1 library. All visitors can read and delete its entries after confirmation.
