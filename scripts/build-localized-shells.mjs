...
import { securitiesMetadata, securitiesArchitectureMetadata } from "../shared/securities/metadata.js";
...
export async function buildLocalizedShells() {
  ...
  for (const [name, metadata] of [["securities", securitiesMetadata], ["securities-architecture", securitiesArchitectureMetadata]]) {
    const base = await readFile(new URL(`${name}.html`, outputDirectory), "utf8");
    const bundles = builtBundleReferences(base);
    const shells = Object.fromEntries(locales.map((locale) => [locale, localizeSecuritiesShell(base, locale, metadata)]));
    for (const shell of Object.values(shells)) {
      if (JSON.stringify(builtBundleReferences(shell)) !== JSON.stringify(bundles)) fail("Securities locale changed built assets");
    }
    await Promise.all([
      writeFile(new URL(`${name}.html`, outputDirectory), shells.en, "utf8"),
      writeFile(new URL(`${name}-vi.html`, outputDirectory), shells.vi, "utf8"),
    ]);
  }
...
export function localizeSecuritiesShell(baseHtml, locale, pageMetadata = securitiesMetadata) {
  const metadata = pageMetadata[locale];
  if (!metadata) fail("Unsupported Securities locale");
  let html = replaceSingleTag(baseHtml, "html", () => true,
    (tag) => setAttribute(tag, "lang", locale, "Securities language"), "Securities HTML");
  html = replaceTitle(html, metadata.title);
  for (const [selectorAttribute, selectorValue, value] of [
    ["name", "description", metadata.description],
    ["property", "og:title", metadata.title], ["property", "og:description", metadata.description],
    ["property", "og:locale", metadata.ogLocale], ["property", "og:locale:alternate", metadata.ogAlternate],
    ["name", "twitter:title", metadata.title], ["name", "twitter:description", metadata.description],
  ]) html = replaceTagAttribute(html, "meta", selectorAttribute, selectorValue, "content", value, selectorValue);
  return html;
}
