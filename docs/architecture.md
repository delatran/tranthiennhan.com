...
The repository also implements [Nhân for Securities](securities.md) at
`/securities`, with its own `src`, `shared`, `worker` and `tests` product modules.
Its local launcher uses a separate D1 schema and an authenticated loopback
PDF/OCR collector. Securities API requests require the explicit local runtime
guard. These source changes do not establish a production deployment or attach
...
The portfolio, Ask Nhân, X Nhân, and X Nhân About share one repository, package, Vite build, and Worker deployment. X Nhân has explicit source boundaries:

| Directory | Ownership |
|---|---|
| `src/xnhan/` | X Nhân and About entry points, React components, hooks, CSS, session state, locale handling, and WebMCP adapters |
| `worker/xnhan/` | Search handler, X Nhân configuration, provider registry and adapters, prompts, ranking, and provider stream parsing |
| `shared/xnhan/` | Runtime independent request and result contracts, locale constants, model display names, and public route helpers |
| `tests/xnhan/` | X Nhân UI, WebMCP, Worker, ranking, provider, and stream regression tests |

Common browser components and infrastructure remain in `src/`. The common Worker router, HTTP helpers, rate limit handling, Ask Nhân, and analytics remain in `worker/`. Answer language helpers shared by both runtimes live in `shared/answer-language.js`.

Keep imports flowing toward shared contracts and common utilities:

- `shared/` must not import from `src/` or `worker/`.
- Browser code must not import from `worker/`; it reaches X Nhân through `/api/xnhan/search`.
- X Nhân backend code must not import from `src/`.
- Portfolio links import `xNhanHref` from `shared/xnhan/routes.js`, without depending on X Nhân UI or locale state.
- The common Worker router enters the X Nhân module through the search handler in `worker/xnhan/search.js`.
- X Nhân provider access, model configuration, prompts, and ranking stay in `worker/xnhan/`.

Ask Nhân retains its existing grounding imports from `src/content.js` in `worker/ask.js` and `worker/ask-facts.js`. Those two dependencies are outside the X Nhân module.

This organization does not introduce a nested Git repository, submodule, or separate release branch. Feature branches merge into `main`; folder ownership defines the module boundary.
