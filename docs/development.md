...
### Nhân for Securities local application

Run `pnpm dev:securities` for the complete loopback Worker, persistent product
database and local PDF/OCR collector. This launcher uses its own configuration
and excludes remote AI and production storage. Follow the
[Securities guide](securities.md) for the guided source case, native WebMCP and
the separately gated, authorized live OpenRouter tests.
...
| `pnpm test:xnhan` | Client build plus focused X Nhân suite in `tests/xnhan/` | None |
...
Start in `src/xnhan/` for the search and About interfaces, `worker/xnhan/` for retrieval and provider behavior, and `shared/xnhan/` for contracts used across runtimes. Keep public links in `shared/xnhan/routes.js` and shared answer language logic in `shared/answer-language.js`. Follow the [module dependency rules](architecture.md#source-ownership-and-dependencies) when introducing imports.

Use a feature branch for a scoped change and merge it into `main`. X Nhân uses the existing root package, build, and deployment; no nested repository or permanent product branch is needed.
...
The focused command builds the client before running the tests in `tests/xnhan/`, so shell assertions always inspect current build output. The full check builds every application entry and discovers Node tests recursively, including the nested X Nhân suite and common routing and repository contracts. Run the focused suite while iterating and the full check before handoff.
