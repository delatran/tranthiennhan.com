...
Keep each change focused on one user visible outcome or one root cause.

Use feature branches that merge into `main`. For X Nhân work, keep UI code in `src/xnhan/`, backend code in `worker/xnhan/`, runtime independent contracts in `shared/xnhan/`, and focused tests in `tests/xnhan/`. Follow the [module dependency rules](docs/architecture.md#source-ownership-and-dependencies) and run `pnpm test:xnhan` while iterating. All applications share the root package, build, and deployment.
