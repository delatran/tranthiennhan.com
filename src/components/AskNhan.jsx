...
import { resolveAnswerLocale } from "../../shared/answer-language.js";
import { content } from "../content.js";
import { createAskTranscriptScroller } from "./ask-scroll.js";
import {
...
export function AskNhan({
  ...
  if (transcriptScrollerRef.current === null) {
    transcriptScrollerRef.current = createAskTranscriptScroller();
  }
  ...
  useEffect(() => {
    if (previousLocaleRef.current !== locale) {
      ...
      transcriptScrollerRef.current.resume();
      setMessages([
        {
  ...
  useLayoutEffect(() => {
    transcriptScrollerRef.current.sync(transcriptRef.current);
  }, [isOpen, messages, typing, modalMode]);

  useEffect(() => {
    const transcript = transcriptRef.current;
    if (!isOpen || !transcript) return undefined;

    const observer = new ResizeObserver(() => {
      transcriptScrollerRef.current.sync(transcript);
    });
      ...
      {isOpen ? (
        ...
        >
          ...
          <div
            ...
            ref={transcriptRef}
            onScroll={(event) =>
              transcriptScrollerRef.current.recordPosition(event.currentTarget)
            }
            role="log"
