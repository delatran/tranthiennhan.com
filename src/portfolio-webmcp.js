...
import { SECURITIES_PATH, securitiesHref } from "../shared/securities/routes.js";
...
export function portfolioOverviewForLocale(locale) {
  ...
  return frozenPublicRecord({
    ...
    securitiesProduct: frozenPublicRecord({
      name: copy.securitiesProduct.name,
      route: SECURITIES_PATH,
    }),
    contactOptions,
  });
}
...
export function portfolioOverviewMatchesDocument(
  ...
  // Overview routes stay canonical; visible links may carry the explicit
  // locale generated by each product's route helper.
  const productRoutes = [overview.product.route, overview.product.aboutRoute].map((route) =>
    [route, xNhanHref(route, overview.locale)],
  );
  if (overview.securitiesProduct) {
    if (visibleText(documentObject.getElementById("securities-product-title")) !== overview.securitiesProduct.name) return false;
    productRoutes.push([overview.securitiesProduct.route, securitiesHref(overview.locale)]);
  }
  return productRoutes.every((allowedHrefs) => anchors.some((anchor) => allowedHrefs.includes(anchor.getAttribute("href"))))
    && overview.contactOptions.every(({ href }) => anchors.some((anchor) => anchor.getAttribute("href") === href));
}
