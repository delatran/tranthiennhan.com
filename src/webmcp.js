...
function normalizeOverviewResult(result) {
  let hasSecuritiesProduct;
  try {
    hasSecuritiesProduct = Object.hasOwn(result, "securitiesProduct");
  } catch {
    throw new TypeError("WebMCP action returned an unsupported result.");
  }
  const overview = readExactWebMcpResultObject(result, [
    ...
    "product",
    "contactOptions",
    ...(hasSecuritiesProduct ? ["securitiesProduct"] : []),
  ]);
  ...
  let securitiesProduct;
  if (hasSecuritiesProduct) {
    const additionalProduct = readExactWebMcpResultObject(overview.securitiesProduct, ["name", "route"]);
    securitiesProduct = Object.freeze({
      name: requireBoundedWebMcpString(additionalProduct.name, { allowedValues: ["Nhân for Securities"] }),
      route: requireBoundedWebMcpString(additionalProduct.route, { allowedValues: ["/securities"] }),
    });
  }
  ...
  return Object.freeze({
    ...
    caseStudies,
    product,
    ...(securitiesProduct ? { securitiesProduct } : {}),
    contactOptions,
  });
}
